...
public class MessagesRepositoryJdbcImpl implements MessagesRepository {
    ...
    public void update(Message message) {
        ...
        String sql = "UPDATE message SET text=?, date_time=?, author=? WHERE message_id=?";
        PreparedStatement pst = null;
        try {
            usersRepositoryJdbc.findById(message.getAuthor().getId());
            pst = connection.prepareStatement( sql );
            pst.setString(1, message.getText());
            pst.setTimestamp(2, message.getDateTime() == null ? null : Timestamp.valueOf(message.getDateTime()));
            pst.setLong(3, message.getAuthor().getId());
            pst.setLong(4, message.getMessage_id());
            pst.executeUpdate();
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public Optional<Message> findById(Long id) throws SQLException {
        ...
        message.setDateTime(rs.getTimestamp("date_time") == null ? null :
